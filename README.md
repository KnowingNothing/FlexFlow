...
## 0. Some tips for building FlexFlow
- if your config.linux can't find CUDNN even if you set the path correctly, you can try to run this script again and it will find the correct path
- use clang++ to build instead of g++. I use g++ (Ubuntu 11.4.0-1ubuntu1~22.04) 11.4.0 and there is something wrong in linking cuda libararies for spec_infer and incr_decoding. I use clang++ v16.0.0 and the build successes
...
## 8. Run SpecInfer example
If you want to use Llama-2, follow the instructions:
```sh
...
```sh
python ../inference/python/spec_infer.py -config-file ./inference/python_test_configs/test_1_config.json
```
If you get an error claiming that you don't have 4 gpus, you can change the json config file to change the number of gpus required.
